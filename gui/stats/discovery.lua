...
--- Does the recipe belong to a crafting category? Factorio 2.1 replaced
--- LuaRecipePrototype::category (one string) with categories (a list, since a
--- recipe may belong to several); 2.0 has only the old field and reading the
--- new one there raises. Probe the list first, fall back to the string.
local function recipe_in_category(recipe, name)
...
function M.proto_lists()
    ...
    ore_set["wood"] = true
    local science_set = science_packs.lab_input_set()
    ...
    for _, recipe in pairs(prototypes.recipe) do
        ...
        if recipe_in_category(recipe, "smelting") and not recipe.hidden_from_flow_stats then
            for _, product in pairs(recipe.products) do
